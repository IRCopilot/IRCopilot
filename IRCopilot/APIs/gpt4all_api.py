...
from IRCopilot.config.chat_config import GPT4ALLConfig
...
class GPT4ALLAPI(LLMAPI):
    ...
    def _chat_completion_fallback(self, history: List) -> str:
        """Fallback method to handle exceptions by completing only the last message."""
        response = self.model.generate(prompt=history[-1], top_k=self.history_length)
        return response

    def _chat_completion(self, history: List) -> str:
        """Main method to generate chat completion with error handling."""
        try:
